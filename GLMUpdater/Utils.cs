...
namespace GLMUpdater
{
	static class Utils
	{
		public static async Task<string> GetLatest()
		{
			...
			{
				HttpResponseMessage response = await httpClient.GetAsync("https://github.com/T-RVSP/Stealth-Luma/releases/latest");
		...
		private static void CompletedHandler(object sender, System.ComponentModel.AsyncCompletedEventArgs e)
		{
			...
			Process.Start("Stealth Luma.exe", "-NoUpdate -PostUpdate");
		}
